...
public class GameManager : MonoBehaviour
{
    ...
    private void SetWinner()
    {
        ...
        // Set camera con snitch
        GameObject.Find("Field Camera").GetComponent<FieldCamera>().LookSnitchAtEnd();

        // Disable ring colliders
        foreach(Transform goal in team1Goals)
        {
            goal.GetComponent<Collider>().enabled = false;
            goal.GetComponent<ScoreScript>().enabled = false;
        }
        foreach(Transform goal in team2Goals)
        {
            goal.GetComponent<Collider>().enabled = false;
            goal.GetComponent<ScoreScript>().enabled = false;
        }
	...
	void Update ()
    {
        ...
        /*
        if(isQuaffleControlled())
        {
            ActiveSignal.transform.position = QuaffleControllingPlayer.transform.position + Vector3.up * 5f;
        }*/
	}
}
