...
public class GoapPlanner_Merodeadores
{
    public Queue<GoapAction_Merodeadores> ElPlan(
        GameObject agente,
        List<GoapAction_Merodeadores > AccionesDisponibles,
        Dictionary<string, object> WorldState,
        Dictionary<string, object> Goal)
    {
        // Limpiar las acciones
        foreach (GoapAction_Merodeadores accion in AccionesDisponibles)
            accion.Reset();

        // De las acciones disponibles checamos cuales se
        // pueden ejecutar con sus precondiciones procedurales
        List<GoapAction_Merodeadores> accionesUsables = new List<GoapAction_Merodeadores>();
        foreach (GoapAction_Merodeadores accion in AccionesDisponibles)
        ...
        List<GoapAction_Merodeadores> resultado = new List<GoapAction_Merodeadores>();
        Node temp = masBarato;
        while(temp!=null)
        {
        ...
        Queue<GoapAction_Merodeadores> colaAcciones = new Queue<GoapAction_Merodeadores>();
        foreach (GoapAction_Merodeadores accion in resultado)
    ...
    private bool ConstruyeGrafo(
        Node inicial, List<Node> arbol, List<GoapAction_Merodeadores> acciones,
        Dictionary<string, object> goal)
    {
        bool encontroSolucion = false;

        // Ir en cada accion y ver si se puede usar
        foreach(GoapAction_Merodeadores accion in acciones)
        {
                ...
                accion.GetPrecondiciones, inicial.estado))
            {
                ...
                else
                {
                    ...
                    List<GoapAction_Merodeadores> accionesRestantes = new List<GoapAction_Merodeadores>();
                    foreach (GoapAction_Merodeadores acc in acciones)
    ...
    private class Node
    {
        public Node padre;
        public float costo;
        public GoapAction_Merodeadores accion;
        public Dictionary<string, object> estado;

        public Node(Node padre, float costo, GoapAction_Merodeadores accion,
            Dictionary<string, object> estado)
        {
            this.padre = padre;
            this.costo = costo;
